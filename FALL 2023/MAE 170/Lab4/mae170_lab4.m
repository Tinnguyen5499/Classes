...
close all;
instrreset;
%% Parameters to set CAN BE ADJUSTED
...
f_max=30; % maximmum frequency to be characterized in Hz
...
% this segment sets the frequency of the generated signal
    s_gen = serial('COM4','BaudRate',115200);
    ...
    s_read = serial('COM13','BaudRate',115200);
    ...
    A_ref=zeros(L,1);
    waittime=2; %set initial wait time before sampling in seconds
    ...
    while flag==0
    out=fscanf(s_read); % reading the serial port
    ind_ref=find(out==',',1); % find index of comma
    ind=find(out==';',1); % find index of semi-colon
    a=str2double(out(1:ind_ref-1)); % get amplitude of output signal
    ...
    fclose(s_read); % closes serial port for "read" arduino
